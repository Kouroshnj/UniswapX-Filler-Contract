...
library OutputsBuilder {
    function single(
        ...
        uint256 amount,
        address recipient
    ) internal pure returns (OutputToken[] memory) {
    ...
    function singlePriority(
        ...
        address recipient
    ) internal pure returns (PriorityOutput[] memory) {
        PriorityOutput[] memory result = new PriorityOutput[](1);
        result[0] = PriorityOutput(
            token,
            amount,
    ...
    function singleV3Dutch(
        ...
        return
            singleV3Dutch(
                V3DutchOutput(token, amount, curve, recipient, minAmount, 0)
            );
    }
